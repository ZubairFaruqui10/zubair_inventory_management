...
var stocks = alasql('SELECT transfer.id AS itransferId, transfer.amount,  stock.id AS stockId , item.code AS itemCode, item.maker, kind.text,  transfer.toDate, whouse.name, \
                    ...
                    WHERE transfer.status = 1 AND transfer.toId = ?', [ thisWId ]  );//stsus 2 maens sent from w2, 0 measn sent from w1, 1 means sent by w2
//we mjoin whouse on fromid beause we nee ither whouse name but we make sure we pick toid as this warehuse in coockie
//transfer.status was 0
                ...
                if (confirm( "Do you want to receive "+cnt + " type of incoming hardware to warehouse.?") ){
                        for (var i = 0; i < 38; i++){
                                ...
                                {           //id INT IDENTITY, fromId INT, toId INT, stockId INT, amount INT, toDate DATE, fromDate DATE, status INT 
                                        alasql('UPDATE transfer SET status = 2 WHERE id = ?', [i]);//item received by w1, manes syatus 3
										//make uapdte to stock and trnascation
                                        var stockObj = alasql('SELECT stockId, amount, fromId FROM transfer WHERE id = ?', [i] )[0] ;
                                        ...
                                        var fromWName =  alasql('SELECT name FROM  whouse WHERE id = ?',[stockObj.fromId ])[0].name;
                                        var transComment = "Transfer received from " + fromWName ;
                                        alasql('INSERT INTO trans VALUES(?,?,?,?,?,?)', [ trans_id, stockObj.stockId, new Date().toJSON().slice(0,10), stockObj.amount, preVal + stockObj.amount, transComment ]);
...
for (var i = 0; i < stocks.length; i++) {
	...
	tmp.push(stock.maker);    //item.make       	
...
//        ,"scrollCollapse": true,
 ,"language": {
                "emptyTable": "You have no incoming product as transfer."
              }
