...
// build sql
var stocks = alasql('SELECT inretailer.id AS inretailerID, item.code AS itemCode , stock.id AS stockId , item.code, item.maker, kind.text, inretailer.saleId, inretailer.productId, inretailer.reason, inretailer.date  \
...
function receiveDamage(){
        ...
        if (cnt > 0){                           
                if (confirm( "Do you want to receive "+cnt + " number of defected hardware to warehouse.?") ){
                        for (var i = 0; i < 38; i++){
                                if (checkedId[i] == 1)
                                {
		 ...
		 "columnDefs": [
            ...
            //}
			 ...
			 ,{
                ...
                ,"language": {
                "emptyTable": "You have no incoming damaged product from retailers."
              }
              ,"order": [[ 7, "asc" ]]
...
$('#stock-tablee tbody').on('click', 'td', function () {
	...
	//made sure it is not the checkbox :p
	{
		...
		if($(this).index() != 0)
			window.location.href = "stock.html?id=" + table.row( this ).data()[1];// + '"';
		//console.log( this  );
